...
def main():
    ...
    parser.add_argument('--log_level', type=int,
                        default=logging.INFO, help='10=DEBUG,20=INFO')
    parser.add_argument('--scope', default='examples')
    parser.add_argument('--stream', default='mystream1')
    parser.add_argument('--location', default='./try.mp4')
    ...
    pravega_plugin_dir = os.path.join(
        script_dir, '..', 'gst-plugin-pravega', 'target', 'debug')
    ...
    plugin_path = ':'.join(
        [python_plugin_dir, pravega_plugin_dir, os.environ['GST_PLUGIN_PATH']])
    ...
    pipeline_description = (
                            f"pravegasrc name=src\n"+
                            f"allow-create-scope=true\n"+
                            f"end-mode=latest ! "+
                            f"filesink name=sink sync=false"
                            )
    ...
    filesink = pipeline.get_by_name('sink')
    if filesink:
        filesink.set_property('location', args.location)
