...
#from predictor_model import Predictor, make_parser
...
class Predictor(object):
    ...
    def inference(self, img):
        ...
        with torch.no_grad():
            t0 = time.time()
            outputs0, outputs1, outputs_2 = self.model(img)
            #logger.info("Infer time: {:.4f}s".format(time.time() - t0))

            if self.decoder is not None:
                outputs1 = self.decoder(outputs1, dtype=outputs1.type())
                outputs2 = self.decoder(outputs2, dtype=outputs1.type())
            ...
            if outputs2[0] is not None:
                outputs2[0][:, 6] = outputs2[0][:, 6] + 2
                outputs = [torch.cat((outputs[0], outputs2[0]), 0)]
            if outputs0[0] is not None:
                outputs0[0][:, 6] = outputs0[0][:, 6] + 4
                outputs = [torch.cat((outputs[0], outputs0[0]), 0)]
            if len(outputs) == 0:
                outputs = [None]
...
class concate_model(nn.Module):
    ...
    def __init__(self, args):
        ...
        ckpt3 = torch.load("./YOLOX_outputs/yolox_tiny-hat/best_ckpt.pth.tar", map_location="cpu")
