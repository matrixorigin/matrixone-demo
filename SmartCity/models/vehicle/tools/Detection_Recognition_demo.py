...
LPRNet_weight_file = "weights/Final_LPRNet_model.pth"
...
class Predictor(object):
    ...
    def visual_LPs(self, output, LPs, img_info, cls_conf=0.35):
        ratio = img_info["ratio"]
        img = img_info["raw_img"]
        ...
        vis_res, _ = self.vis_LPs(img, bboxes, scores, cls, LPs, width, cls_conf, self.cls_names)
        return vis_res

    def vis_LPs(self, img, boxes, scores, cls_ids, license_plates, cal_para, conf=0.5, class_names=None):
        LP_list = []
        vehicle_list = []
        num_LP = 0
        num_veh = 0
        for i in range(len(boxes)):
            if int(cls_ids[i]) != 5:
                ...
                output_veh = {  # 车辆
                    "veh_x1": box[0].numpy().astype(float),
                    "veh_y1": box[1].numpy().astype(float),
                    "veh_x2": box[2].numpy().astype(float),
                    "veh_y2": box[3].numpy().astype(float),
                    "veh_confidence": score.numpy().astype(float),
                    "plate": {''},
...
def image_demo(predictor, vis_folder, path, current_time, save_result):
    ...
    for image_name in files:
        ...
        time2 = time.time()
        result_image = predictor.visual_LPs(outputs, LPs, img_info, predictor.confthre)
        time3 = time.time()
